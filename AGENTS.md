...
- Toda mudança em collections/globals em dev exige `pnpm payload migrate:create` ANTES do deploy prod. Dev/staging usam `push: true` (auto-sync); prod usa migrate explícito. Sem snapshot capturado, prod quebra (caso real: 5 page Globals pós-MVP precisaram migration manual `20260430_pages_globals.ts` capturada via `pg_dump --schema-only`).
- `payload migrate:create` exige TTY interativo (drizzle-kit prompts sobre rename detection). Não roda em CI ou via heredoc — só interativo no shell de dev.
- Pages do `(site)/*` SEMPRE com `export const dynamic = 'force-dynamic'`. Sem isso, Next prerender static → cache `s-maxage=31536000` → mudanças no admin só visíveis após rebuild. Trade-off: perde static opt mas correto pra CMS-driven editor flow.
...
- Página fixa do catálogo: `/blog/catalogo-open-source`.
- Cada projeto catalogado vira um `Article` publicado em `/blog/[slug]`; a entrada em `OpenSourceCatalogEntries` só aponta para esse post.
- Posts do catálogo devem ser substanciais e didáticos, úteis para iniciantes e profissionais experientes.
- Conteúdo em PT-BR real: manter acentos e cedilhas no título, excerpt, corpo, metadados de fonte e resumo do catálogo. Não publicar texto sem diacríticos.
- Subtítulos do corpo precisam ser nós Lexical `heading` (`h2`/`h3`), não parágrafos com texto solto. Isso é obrigatório para a página renderizar com hierarquia visual.
- Validar antes de considerar pronto: artigo público com headings destacados, bloco de código com botão Copiar quando houver comandos, cover OG com acentuação correta e entrada visível no índice.
- O fallback de cover usa `/og/[slug]?v=<updatedAt>` para evitar cache antigo; não remover o versionamento sem substituir por outra estratégia de invalidação.
...
- **Site `(site)/*` pages SEMPRE `dynamic = 'force-dynamic'`**. Sem isso, Next prerendera static → conteúdo do build fica cacheado com `s-maxage=31536000` → edits no admin Globals/Collections só viram visíveis após rebuild. Trade-off: site faz query no DB a cada request, mas é exatamente o que se espera de um site CMS-driven com editor frequente. Aplica a `page.tsx` (home), `produtos/`, `servicos/`, `sobre/`, `contato/`, `blog/`, `blog/[slug]/`.
- **Schema dev usa `push: true`, prod usa `migrate explícito`**. Toda mudança em `src/collections/*.ts` ou `src/globals/*.ts` exige `pnpm payload migrate:create --name <nome>` ANTES de fazer deploy prod. Senão prod quebra com `relation does not exist` (caso real em 2026-04-30: 5 page Globals novos não tinham snapshot, foi necessário migration manual `20260430_pages_globals.ts` reconstruída via `pg_dump --schema-only` do staging). `payload migrate:create` exige TTY interativo — só roda no shell dev, não em CI.
- **Build prod precisa migrate ANTES de next build**. `Dockerfile.prod` faz `RUN pnpm payload migrate` antes de `RUN pnpm build` (idempotente). Razão: prerender de várias rotas (sitemap, RSS, /blog list) hita Payload Local API → schema precisa existir. Sem isso, build falha com 42P01.
