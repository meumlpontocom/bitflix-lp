...
### 2026-04-29 — Fase 5: bug crítico `docker compose restart` NÃO recarrega `.env`
- **Sintoma:** após mudar `PAYLOAD_PUBLIC_SERVER_URL` no `.env` de `http://localhost:3023` pra `https://staging.cms.bitflix.com.br` e fazer `docker compose restart bitflix-lp-app`, container continuou com env var STALE (`localhost:3023`). Resultado: Payload v3 derivou `serverURL`, `cors`, `csrf` de URL errada → browser admin com Origin `https://staging.cms.bitflix.com.br` foi rejeitado por CSRF allowlist em `extractJWT.cookie` strategy → `req.user` undefined em todos POST/PATCH/DELETE → 403 "You are not allowed to perform this action" em qualquer save no admin (autores, articles, globals, etc).
- **Causa:** Docker Compose `.env` substitution acontece no `up`/`up --force-recreate`, NÃO no `restart`. `restart` apenas reinicia processo do container existente, mantendo env vars antigas.
- **Diagnóstico:** payload.config.ts lia `process.env.PAYLOAD_PUBLIC_SERVER_URL` com valor stale, montou `csrf: ['http://localhost:3023']`. Cookie strategy do Payload v3 (`node_modules/payload/dist/auth/extractJWT.js` linha 19-29): se Origin presente e não está em csrf list, retorna null → JWT não extraído → user null. Browser sempre envia Origin em mutations.
- **Fix definitivo:** `docker compose up -d --force-recreate bitflix-lp-app` após qualquer mudança em `.env`. Confirmar com `docker compose exec -T bitflix-lp-app printenv PAYLOAD_PUBLIC_SERVER_URL`.
- **How to apply:** depois de editar `.env`, NUNCA usar `docker compose restart`. Sempre `docker compose up -d --force-recreate <service>`. Documentar no `docs/INFRA.md` ou criar Makefile com target `env-reload`.
- **Side fixes durante diagnóstico:** (a) middleware `ADMIN_ONLY_PATHS` reduzido de `['/admin', '/api/payload', '/api/graphql', '/api/graphql-playground']` pra `['/admin', '/api']` — Payload REST API gerada via `(payload)/api/[...slug]/route.ts` cobre `/api/users/*`, `/api/authors/*` etc, todos precisam ser admin-host-only. (b) Layout root duplicado (`src/app/layout.tsx` + `src/app/(payload)/layout.tsx` ambos emitindo `<html>+<body>`) refatorado: `app/layout.tsx` deletado, html+body movido pra `app/(site)/layout.tsx`, `error.tsx`/`not-found.tsx` movidos pra `app/(site)/`, `global-error.tsx` permanece em `app/`. Estrutura canônica Payload v3 + Next 15. (c) `useSessions: false` em Users mantido pra simplicidade (sessions feature exige sid em JWT validar contra DB; manter desabilitado evita estado extra). Tabela `users_sessions` foi droppada manualmente.
- **Decisão Payload config:** `cors: { origins: [serverURL] }` + `csrf: [serverURL]` explícitos com fallback `[]`. Em prod (tomahawk), `PAYLOAD_PUBLIC_SERVER_URL=https://bitflix.com.br` faz tudo derivar correto.
